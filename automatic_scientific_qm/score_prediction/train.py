...
def train(config):
    with isolate_rng():
        # Model Directory
        model_directory = os.path.join(config["model_directory"],"score_prediction_model")
        ...
        # Load dataset
        train_dataloader, val_dataloader, test_dataloader = get_data(config["data"])
        ...
        if config["trainer"]["load"]["load"]:
            ...
            if config["data"]["pairwise_comparison"]:
                model = PairwiseComparison(config["trainer"],config["data"]["use_context"])
            else:
                model = ScorePrediction(config["trainer"],config["data"]["use_context"],binary=False)
        ...
        if config["data"]["pairwise_comparison"]:
            ...
            best_model = ScorePredictor.load_from_checkpoint(best_model_path)
...
def get_experiment_name(config):
    ...
    experiment_name += "_pairwise" if config["data"]["pairwise_comparison"] else "_exact"
    experiment_name += "_context" if config["data"]["use_context"] else "_no_context"
    experiment_name += f"_{config['data']['score_to_predict']}"
    experiment_name += f"_{config['data']['paper_representation']}"
    if config["data"]["topic_id"] >= 0:
        experiment_name += f"_{config['data']['topic_id']}"
...
if __name__=="__main__":
    ...
    parser.add_argument("--percentage", type=float, default=None, help="Percentage of training data to use")
    parser.add_argument("--seed", type=int, default=None, help="Seed for reproducibility")
    parser.add_argument("--dataset", type=str, default=None, help="Dataset to use")
    parser.add_argument("--pairwise", action="store_true", help="Use pairwise comparison")
    parser.add_argument("--score", type=str, default=None, help="Score to predict")
    parser.add_argument("--paper_representation", type=str, default=None, help="Representation of the paper")
    parser.add_argument("--topic_id", type=str, default=None, help="Topic id to sample the data by")
